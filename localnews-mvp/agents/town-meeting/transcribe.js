...
async function main() {
  ...
  if (!inputPath) {
    console.error('Usage: node transcribe.js <video_path|video_id> [output_path]');
    console.error('  Accepts: /path/to/video.mp4, /path/to/video.vtt, or just video_id');
    process.exit(1);
  }

  // Handle video ID input (just a number) - resolve to data directory
  const dataDir = path.join(__dirname, '../../data/swagit');
  if (/^\d+$/.test(inputPath)) {
    const videoId = inputPath;
    // Check for VTT first, then video
    if (fs.existsSync(path.join(dataDir, `${videoId}.vtt`))) {
      inputPath = path.join(dataDir, `${videoId}.vtt`);
      console.log(`📂 Resolved video ID ${videoId} to VTT: ${inputPath}`);
    } else if (fs.existsSync(path.join(dataDir, `${videoId}.mp4`))) {
      inputPath = path.join(dataDir, `${videoId}.mp4`);
      console.log(`📂 Resolved video ID ${videoId} to MP4: ${inputPath}`);
    } else {
      console.error(`❌ No VTT or MP4 found for video ID: ${videoId}`);
      process.exit(1);
    }
  }

  // Determine video name and work directory
  const ext = path.extname(inputPath);
  const videoName = path.basename(inputPath, ext);
  ...
  // If input is already a VTT, use it directly
  const potentialVttPath = ext === '.vtt' ? inputPath : path.join(workDir, `${videoName}.vtt`);
  const videoPath = ext === '.vtt' ? path.join(workDir, `${videoName}.mp4`) : inputPath;
  if (fs.existsSync(potentialVttPath)) {
    ...
    if (vttContent.trim().length > 0) {
      ...
      console.log('⏩ Skipping Whisper API transcription (no API cost!)');

      const { fullText, segments } = parseVtt(vttContent);

      // Estimate duration from segments
      const lastSegment = segments[segments.length - 1];
      const estimatedDuration = lastSegment ? Math.round(lastSegment.startSeconds / 60) : 0;

      const output = {
        videoId: videoName,
        sourceFile: potentialVttPath,
        transcribedAt: new Date().toISOString(),
        durationMinutes: estimatedDuration,
        fullText,
        segments,
        source: 'vtt' // Flag that this came from VTT, not Whisper
      };
  ...
  // Need to use Whisper API - check for API key and video file
  if (!process.env.OPENAI_API_KEY) {
    console.error('❌ OPENAI_API_KEY not found in environment');
    console.error('   VTT transcript not available, cannot proceed without API key.');
    process.exit(1);
  }

  if (!fs.existsSync(videoPath)) {
    console.error(`❌ Video file not found: ${videoPath}`);
    console.error('   Cannot transcribe without video file.');
    process.exit(1);
  }
